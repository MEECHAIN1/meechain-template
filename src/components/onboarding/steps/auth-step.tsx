...
export function AuthStep({ onNext, onPrev }: AuthStepProps) {
  ...
  const handleAuth = async (provider: string) => {
    setIsLoading(true);
    // Mock user payload we always fall back to when API is unavailable.
    const mockUserData = {
      provider,
    ...
    const completeAuth = (userPayload: unknown, message?: string) => {
      const userRecord = (userPayload as Record<string, unknown>) ?? {};
      const userId = typeof userRecord.id === "string" ? userRecord.id : mockUserData.socialId;
      ...
      updateOnboardingData({ 
        ...
        user: {
          id: userId,
          email: userRecord.email ?? mockUserData.email,
          firstName: userRecord.firstName ?? mockUserData.firstName,
          lastName: userRecord.lastName ?? mockUserData.lastName,
          profileImageUrl: userRecord.profileImageUrl ?? mockUserData.profileImageUrl,
        },
      });
      ...
      toast({
        ...
        description: message ?? `เข้าสู่ระบบด้วย ${provider} เรียบร้อยแล้ว`,
      });
    };
    ...
    try {
      ...
      completeAuth(data?.user ?? data);
    } catch (error) {
      console.warn("Auth endpoint unavailable, using local mock user", error);
      completeAuth(mockUserData, `โหมดจำลอง: เข้าสู่ระบบด้วย ${provider}`);
    } finally {
      setIsLoading(false);
    }
  };
