...
// ส่วนนี้จะถูกนำไปใช้ในไฟล์ Tests
describe("MeeChain Badge System", function () {
  ...
  describe("MeeBadgeNFT", function () {
    it("Should mint badge NFT with correct properties and track user ownership", async function () {

         // 2. Mint Badge ให้ user1 (สมมติว่าเป็น tokenId 0)
    await badgeNFT.connect(owner).mintBadge(
        user1.address, // ผู้รับ
        "Test Badge",
        "Test Description",
        0, 0, "https://test.com/badge.json", false, ""
    );

    // 3. ตรวจสอบคุณสมบัติของ Badge (tokenId 0)
    ...
    // 4. ตรวจสอบว่า Badge ถูกบันทึกในรายการของผู้ใช้หรือไม่
    const userBadges = await badgeNFT.getUserBadges(user1.address);
    expect(userBadges.length).to.equal(1);   // user1 ควรมี 1 Badge
    expect(userBadges[0]).to.equal(0);        // และ Badge ชิ้นนั้นคือ tokenId 0
}); // <--- ปิด it block
