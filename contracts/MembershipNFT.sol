...
contract MembershipNFT is ERC721, ERC721URIStorage, Ownable, Pausable, IERC721Mintable {
    ...
    /**
     * @dev Update tier metadata (only owner)
     */
    function updateTierMetadata(uint8 tier, string memory name, string memory uri) external onlyOwner {
        require(tier >= 1 && tier <= 4, "Invalid tier");
        tierNames[tier] = name;
        tierURIs[tier] = uri;
        emit TierMetadataUpdated(tier, name, uri);
    }
    
    /**
     * @dev Get tier name
     */
    function getTierName(uint8 tier) external view returns (string memory) {
        return tierNames[tier];
    }
    
    /**
     * @dev Get all user's tiers
     */
    function getUserTiers(address user) external view returns (uint8[] memory) {
        uint8[] memory tiers = new uint8[](4);
        uint8 count = 0;
        
        for (uint8 i = 1; i <= 4; i++) {
            if (userHasTier[user][i]) {
                tiers[count] = i;
                count++;
            }
        }
